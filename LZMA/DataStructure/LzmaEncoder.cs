	...
	public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
	{
		...
		void Create()
		{
			...
			_matchFinder.Create(_dictionarySize, kNumOpts, _numFastBytes,
				Base.kMatchMaxLen - _numFastBytes);
		...
		UInt32 GetOptimum(UInt32 position, out UInt32 backRes)
		{
			...
			while (true)
			{
				...
				*/
				...
				numAvailableBytes = Math.Min(kNumOpts - 1 - cur, numAvailableBytes);
				...
				for (UInt32 repIndex = 0; repIndex < Base.kNumRepDistances; repIndex++)
				{
					...
					for (lenTest = 0; lenTest < numAvailableBytes; lenTest++)
						...
						if (_matchFinder.GetIndexByte((Int32)lenTest - 1) != _matchFinder.GetIndexByte((Int32)lenTest - (Int32)backOffset - 1))
							break;
